...
###################################################################################################
# Update Jetpack 4.5.1 software for Jetson AGX Xavier and install OpnCV&CUDA, tensorflow 2.x.x, deepstream
# You can change variables TF_VERSION, NV_VERSION and DEEPSTREAM_VERSION
# to configure the desired tensorflow 2.x.x and deepstream versions
###################################################################################################
...
TF_VERSION=2.2.0
NV_VERSION=20.08
DEEPSTREAM_VERSION=5.1

# Jetson Xavier Jetpack 4.5.1
...
sudo apt-get install deepstream-$DEEPSTREAM_VERSION
...
echo -e "Building OpenCV&CUDA. This will take several hours to compile...\n\n"
...
# Fix for possible error "Illegal instruction(core dumped)" while importing cv2 in python
# The issue was found for now on 4.5.1 JetPack
